...
namespace MarvelUWP
{
    public class MarvelFacade
    {
            ...
            try
            {
                ...
                foreach (var character in characters)
                {
                        ...
                        && character.thumbnail.path != ImageNotAvailablePath)
                    {
                        ...
                        character.thumbnail.large = String.Format("{0}/standard_xlarge.{1}", character.thumbnail.path, character.thumbnail.extension);
        ...
        public static async Task PopulateMarvelComicsAsync(int characterId, ObservableCollection<ComicBook> marvelComics)
        {
            try
            {
                ...
                foreach (var comic in comics)
                {
                    ...
                    {
                        comic.thumbnail.small = String.Format("{0}/portrate_medium.{1}", comic.thumbnail.path, comic.thumbnail.extension);
        ...
        private static async Task<CharacterDataWrapper> GetCharacterDataWrapperAsync()
        {
            ...
            //Respsone -> string / json -> deserialize
